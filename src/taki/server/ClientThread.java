...
public class ClientThread extends Thread {
	...
	public void run() {
		...
		while (keepGoing) {
			...
			try {
				Object clientMsg = _sInput.readObject();
				if (clientMsg instanceof ChatMessage) {
					_cm = (ChatMessage) clientMsg;
					keepGoing = handleChatMsg();
				} else if (clientMsg instanceof GameMessage) {
					handleGameMsg((GameMessage) clientMsg);
				}
			} catch (IOException e) {
	...
	private void handleGameMsg(GameMessage clientMsg) {
		boolean bIsValid = false;
		switch (clientMsg.getClientAction()) {
		case CHOSE_CARD: {

			bIsValid = _server.getGameLogic().playerChoseCard(clientMsg.getPlayerName(), clientMsg.getCard());
			break;
		}
		case TAKE_CARD_FROM_DECK: {
			bIsValid = _server.getGameLogic().takeNewCardFromDeck(clientMsg.getPlayerName());
			break;
		}
		default: {
			break;
		...
		if (bIsValid) {
			_server.broadcast(new GameMessage(_server.getGameLogic().getGameState()));
		}
	}

	private boolean handleChatMsg() {
